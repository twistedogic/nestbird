...
When NetBird is disconnected, the watcher SHALL execute `netbird up` to reconnect.

#### Scenario: Reconnect succeeds
- **WHEN** NetBird is disconnected and `netbird up` succeeds
- **THEN** watcher logs "Connected" and returns to polling

#### Scenario: Reconnect fails
- **WHEN** NetBird is disconnected and `netbird up` fails
